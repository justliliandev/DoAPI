...
public class StandardBlockEntityRenderer implements BlockEntityRenderer<StandardBlockEntity>
{
    ...
    public void render(StandardBlockEntity standard, float f, PoseStack poseStack, MultiBufferSource multiBufferSource, int i, int j) {
        ...
        else
        {
            ...
            float rotation;
            if (!(blockState.getBlock() instanceof StandardWallBlock))
            {
                poseStack.translate(0.5, 0.5, 0.5);
                rotation = (float)(-blockState.getValue(StandardBlock.ROTATION) * 360) / 16.0f;
                poseStack.mulPose(Axis.YP.rotationDegrees(rotation));
                this.pole.visible = true;
            }
            else {
                poseStack.translate(0.5, -0.1666666716337204, 0.5);
                rotation = -blockState.getValue(StandardWallBlock.FACING).toYRot();
                poseStack.mulPose(Axis.YP.rotationDegrees(rotation));
